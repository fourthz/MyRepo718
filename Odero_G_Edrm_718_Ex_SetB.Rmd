...
test_means<-colMeans(hsb2[,c(7:11)])
test_means
```
...
Scores_data<-array(c(First_25_private,First_25_public),dim=c(25,11,2))
Scores_data
```
...
```{r}
col_names<-c("id","female",	"race",	"ses",	"schtyp",	"prog",	
...
Math_quartiles<-quantile(hsb2$math,probs=c(0.25,0.5,0.75))
Math_quartiles
```

```{r}
# lets check with the boxplot 
...
rand_bins= cut(rand_norms,breaks=c(0,80,100,120,max(rand_norms))) # specify the breaks
levels(rand_bins) = c("0-80","80-100","100-120","0ver 120")
```
...
xout <- as.data.frame(table(rand_bins))
xout <- transform(xout)
xout
```
