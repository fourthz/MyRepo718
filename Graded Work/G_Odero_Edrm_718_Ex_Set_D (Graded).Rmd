...
**The reason that I require use of the "here" library and the construction of**
**a data folder in your repo is so that you will put your data in that folder**
**and then use the here() function to read the data. That way I don't have**
**to change your code in order to read the data. (-4)**
...
```{r}
secret_code<- Secret_Code <- read.csv2(here("Data", "Secret Code.csv"),sep = ",",header = FALSE)
...
sample_data<- read.csv(here("Data", "Sample Data.csv"), sep="")
```
